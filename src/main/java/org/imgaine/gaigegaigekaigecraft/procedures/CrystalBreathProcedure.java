...
public class CrystalBreathProcedure {
   public CrystalBreathProcedure() {
   }
   ...
   public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
      if (entity != null) {
         ...
         if (entity.getPersistentData().getDouble("cnt2") < 10.0) {
            ...
            y_pos = entity.getY() + (double)entity.getBbHeight() * 0.75 + Math.sin(pitch) * -1.0 * distance;
            z_pos = entity.getZ() + Math.sin(yaw) * Math.cos(pitch) * distance;
            ParticleGeneratorProcedure.execute(world, range * 0.5, 12.0 * range * CNT6, 45.0, 1.0 * CNT6, x_pos, x_pos + x_power, y_pos, y_pos + y_power, z_pos, z_pos + z_power, "minecraft:large_smoke");
