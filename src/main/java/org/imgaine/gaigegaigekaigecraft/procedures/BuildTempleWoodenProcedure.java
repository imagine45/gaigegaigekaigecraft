...
public class BuildTempleWoodenProcedure {
   public BuildTempleWoodenProcedure() {
   }
   ...
   public static void execute(LevelAccessor world, double x, double y, double z) {
      ...
      double sy = 0.0;
      double sz = 0.0;
      if (world.getEntitiesOfClass(CursedSpiritGrade13Entity.class, AABB.ofSize(new Vec3(x, y, z), 64.0, 64.0, 64.0), (e) -> true).isEmpty()) {
         ...
         for(int index0 = 0; index0 < 20; ++index0) {
            ...
            for(int index1 = 0; index1 < 256; ++index1) {
               ...
               for(int index2 = 0; index2 < 24; ++index2) {
                  if (world.isEmptyBlock(BlockPos.containing(x_pos, y_pos, z_pos)) && !world.isEmptyBlock(BlockPos.containing(x_pos, y_pos - 1.0, z_pos)) && !world.getBlockState(BlockPos.containing(x_pos, y_pos - 1.0, z_pos)).is(BlockTags.create(new ResourceLocation("minecraft:slabs")))) {
                     SUCCESS = true;
            ...
            if (!boss_flag) {
               ...
               if (world instanceof ServerLevel) {
                  ServerLevel _level = (ServerLevel)world;
                  Entity entityToSpawn = ((EntityType)JujutsucraftModEntities.CURSED_SPIRIT_GRADE_13.get()).spawn(_level, BlockPos.containing(x_pos, y_pos, z_pos), MobSpawnType.MOB_SUMMONED);
               ...
               for(Entity entityiterator : world.getEntitiesOfClass(Entity.class, (new AABB(_center, _center)).inflate(0.5), (e) -> true)) {
                  if (entityiterator instanceof CursedSpiritGrade13Entity && !entityiterator.level().isClientSide() && entityiterator.getServer() != null) {
               ...
               for(int index6 = 0; index6 < 256; ++index6) {
                  kind = (double)Math.round(Math.random() * 18.0);
                  if (kind != 7.0) {
                     break;
                  }
               }
