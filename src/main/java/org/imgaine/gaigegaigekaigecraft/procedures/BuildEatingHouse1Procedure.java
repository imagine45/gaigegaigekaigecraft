...
public class BuildEatingHouse1Procedure {
   public BuildEatingHouse1Procedure() {
   }
   ...
   public static void execute(LevelAccessor world, double x, double y, double z) {
      ...
      double sy = 0.0;
      double sz = 0.0;
      if (world.getEntitiesOfClass(CursedSpiritGrade28Entity.class, AABB.ofSize(new Vec3(x, y, z), 64.0, 64.0, 64.0), (e) -> true).isEmpty()) {
         ...
         for(int index0 = 0; index0 < 20; ++index0) {
            ...
            for(int index1 = 0; index1 < 256; ++index1) {
               x_pos = x + (Math.random() - 0.5) * 40.0 * 0.8;
               y_pos = y - 5.0;
               z_pos = z + (Math.random() - 0.5) * 40.0 * 0.8;
               if (!boss_flag) {
                  entity_height = 6.0;
                  entity_width = 6.0;
               } else {
            ...
            if (!boss_flag) {
               ...
               if (world instanceof ServerLevel) {
                  ServerLevel _level = (ServerLevel)world;
                  Entity entityToSpawn = ((EntityType)JujutsucraftModEntities.CURSED_SPIRIT_GRADE_28.get()).spawn(_level, BlockPos.containing(x_pos, y_pos, z_pos), MobSpawnType.MOB_SUMMONED);
