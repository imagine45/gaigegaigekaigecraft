...
public class BuildGate1Procedure {
   ...
   public static void execute(LevelAccessor world, double x, double y, double z) {
      ...
      double sy = 0.0;
      double sz = 0.0;
      if (world.getEntitiesOfClass(CursedSpiritGrade03Entity.class, AABB.ofSize(new Vec3(x, y, z), 64.0, 64.0, 64.0), (e) -> true).isEmpty() && world.getEntitiesOfClass(CursedSpiritGrade29Entity.class, AABB.ofSize(new Vec3(x, y, z), 64.0, 64.0, 64.0), (e) -> true).isEmpty()) {
         ...
         if (index0 < 1) {
            ...
            if (Math.random() < 0.1) {
               if (world instanceof ServerLevel) {
                  ServerLevel _level = (ServerLevel)world;
                  Entity entityToSpawn = ((EntityType)JujutsucraftModEntities.CURSED_SPIRIT_GRADE_03.get()).spawn(_level, BlockPos.containing(x_pos, y_pos, z_pos), MobSpawnType.MOB_SUMMONED);
               ...
               for(Entity entityiterator : world.getEntitiesOfClass(Entity.class, (new AABB(_center, _center)).inflate(0.5), (e) -> true)) {
                  if (entityiterator instanceof CursedSpiritGrade03Entity && !entityiterator.level().isClientSide() && entityiterator.getServer() != null) {
