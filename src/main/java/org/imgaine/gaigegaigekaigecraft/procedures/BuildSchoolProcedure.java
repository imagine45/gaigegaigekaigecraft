...
public class BuildSchoolProcedure {
   public BuildSchoolProcedure() {
   }
   ...
   public static void execute(LevelAccessor world, double x, double y, double z) {
      ...
      double sy = 0.0;
      double sz = 0.0;
      if (world.getEntitiesOfClass(CursedSpiritGrade02Entity.class, AABB.ofSize(new Vec3(x, y, z), 64.0, 64.0, 64.0), (e) -> true).isEmpty()) {
         if (world instanceof Level) {
            Level _level = (Level)world;
            if (!_level.isClientSide()) {
               _level.playSound((Player)null, BlockPos.containing(x, y, z), (SoundEvent)ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("gaigegaigekaigecraft:school_bell")), SoundSource.NEUTRAL, 2.0F, 1.0F);
            } else {
               _level.playLocalSound(x, y, z, (SoundEvent)ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("gaigegaigekaigecraft:school_bell")), SoundSource.NEUTRAL, 2.0F, 1.0F, false);
            }
         }
         ...
         for(int index0 = 0; index0 < 48; ++index0) {
            ...
            for(int index1 = 0; index1 < 256; ++index1) {
               entity_height = 2.0;
               entity_width = 2.0;
               x_pos = x + (Math.random() - 0.5) * 90.0 * 0.8;
               y_pos = y - 24.0;
               z_pos = z + (Math.random() - 0.5) * 90.0 * 0.8;
            ...
            if (!boss_flag) {
               ...
               if (world instanceof ServerLevel) {
                  ServerLevel _level = (ServerLevel)world;
                  Entity entityToSpawn = ((EntityType)JujutsucraftModEntities.CURSED_SPIRIT_GRADE_02.get()).spawn(_level, BlockPos.containing(x_pos, y_pos, z_pos), MobSpawnType.MOB_SUMMONED);
               ...
               for(Entity entityiterator : world.getEntitiesOfClass(Entity.class, (new AABB(_center, _center)).inflate(0.5), (e) -> true)) {
                  if (entityiterator instanceof CursedSpiritGrade02Entity && !entityiterator.level().isClientSide() && entityiterator.getServer() != null) {
               ...
               if (Math.random() < 0.9) {
                  ...
                  for(int index6 = 0; index6 < 256; ++index6) {
                     kind = (double)Math.round(Math.ceil(Math.random() * 19.0));
                     if (kind != 7.0) {
                        break;
                     }
                  }
                  ...
                  for(int index7 = 0; index7 < 256; ++index7) {
                     kind = (double)Math.round(Math.ceil(Math.random() * 12.0));
                     if (kind == 4.0 || kind == 5.0 || kind == 9.0 || kind == 10.0 || kind == 11.0 || kind == 12.0) {
                        break;
                     }
                  }
               }
