...
public class AIKurourushiProcedure {
   public AIKurourushiProcedure() {
   }
               ...
               if (entity.getPersistentData().getDouble("cnt_x") > 10.0 && entity.getPersistentData().getDouble("skill") == 0.0) {
                  ...
                  if (LogicStartProcedure.execute(entity)) {
                     ...
                     if (Math.random() < 0.3 && distance < 12.0) {
                        rnd = 5.0;
                        tick = 50.0;
                     } else if (Math.random() < 0.3 && distance > 6.0) {
                        rnd = 6.0;
                        tick = 75.0;
                     } else if (Math.random() < 0.2 && distance > 6.0) {
                        rnd = 7.0;
                        tick = 100.0;
                     } else if (Math.random() < 0.4 && distance < 24.0) {
                        rnd = 8.0;
                        tick = 50.0;
                     }
                  }
                  ...
                  if (rnd > 0.0) {
                     entity.getPersistentData().putDouble("skill", 2300.0 + rnd);
                     if (entity instanceof LivingEntity) {
                        LivingEntity _entity = (LivingEntity)entity;
                        if (!_entity.level().isClientSide()) {
