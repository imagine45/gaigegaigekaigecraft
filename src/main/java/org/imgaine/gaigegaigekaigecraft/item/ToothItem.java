...
public class ToothItem extends Item {
   ...
   public void releaseUsing(ItemStack itemstack, Level world, LivingEntity entity, int time) {
      if (!world.isClientSide() && entity instanceof ServerPlayer player) {
         ...
         if (player.getAbilities().instabuild || stack != ItemStack.EMPTY) {
            BulletToothEntity projectile = BulletToothEntity.shoot(world, entity, world.getRandom());
   ...
   private ItemStack findAmmo(Player player) {
      ItemStack stack = ProjectileWeaponItem.getHeldProjectile(player, (e) -> e.getItem() == BulletToothEntity.PROJECTILE_ITEM.getItem());
      if (stack == ItemStack.EMPTY) {
         for(int i = 0; i < player.getInventory().items.size(); ++i) {
            ItemStack teststack = (ItemStack)player.getInventory().items.get(i);
            if (teststack != null && teststack.getItem() == BulletToothEntity.PROJECTILE_ITEM.getItem()) {
               stack = teststack;
               break;
            }
         }
