...
import org.imgaine.gaigegaigekaigecraft.procedures.AnswerJudgemanProcedure;
...
@EventBusSubscriber
public class JudgemanAnswerCommand {
   public JudgemanAnswerCommand() {
   }

   @SubscribeEvent
   public static void registerCommand(RegisterCommandsEvent event) {
      event.getDispatcher().register((LiteralArgumentBuilder)Commands.literal("judgeman_answer").then(Commands.argument("answer", StringArgumentType.word()).executes((arguments) -> {
         ...
         AnswerJudgemanProcedure.execute(world, arguments, entity);
         return 0;
      })));
   }
}
